...
p1 <- ggplot() + theme_classic() + 
  ...
  scale_x_continuous(expand = c(0,0), limits = c(150.3,152.5)) + scale_y_continuous(expand = c(0,0), limits = c(-35.5,-32.8)) +
  ...
  scalebar(NULL, x.min =150.3, x.max = 152.5, y.min = -35.5, y.max = -32.8,
           dist = 30, dist_unit = "km",
           transform = T, model = "WGS84", st.bottom = F,
           anchor = c("x" = 152.3,"y"= -35.48)) +
  north(scale = 0.1, symbol = 3, anchor = c("x" = 152.9,"y"= -32.05),
               x.min =150.3, x.max = 152.5, y.min = -35.5, y.max = -32.8)+
...
pol<-data.frame(xmin=150,xmax=153 ,ymin=-36 ,ymax=-32)
...
p2<-ggplot()+geom_polygon(data=Aus_coast, aes(x=long, y = lat, group = group), fill = "gray60", colour = "gray60") +
  ...
  geom_rect(data = pol, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), alpha=0, colour="red", size = 0.7, linetype=1)+
...
v2<-viewport(width = 0.2, height = 0.25, x = 0.348, y = 0.903, clip = "off") #plot area for the inset map
print(p1,vp=v1) 
print(p2,vp=v2)
dev.off()

pdf(file="Figure 1.pdf",w=7,h=7)
grid.newpage()
v1<-viewport(width = 1, height = 1, x = 0.5, y = 0.5) #plot area for the main map
v2<-viewport(width = 0.2, height = 0.25, x = 0.33, y = 0.903, clip = "off") #plot area for the inset map
print(p1,vp=v1) 
print(p2,vp=v2)
dev.off()
...
pBB <- ggplot(Bb) + theme_classic() + 
  ...
  theme(axis.title.x = element_blank(),
        ...
        #strip.background = element_rect(colour = NULL),
        legend.justification=c(1,0), legend.position="none", legend.direction = "vertical",
        #panel.border = element_rect(colour = "black", fill=NA, size = 1),
        #legend.key.size = unit(1, "cm"),
  ...
  scalebar(NULL, x.min =boxes$x_min[3], x.max = boxes$x_max[3], y.min = boxes$y_min[3],
           ...
           anchor = c("x" = 151.247,"y"= -33.973), border.size = 0.5, st.size = 1.5) +
  #geom_point(data = dots, aes(x = Longitude, y = Latitude*-1, col = cols), size = 2) +
  #scale_shape_manual(values = 1, label ="Larval Fish \nSample") +
  geom_point(data = Bb_reefs, aes(x = Long, y = Lat, shape = Type), size = 1) #+
  #geom_text(data = boxes, aes(x = x_min, y = y_min, label = Estuary), 
  #          nudge_x = c(0.75,0.55,0.8))# +

pBB

ggsave("Botany Bay map.pdf", height = 6, width = 7, units = "cm")
...
ggsave("Lake Macquarie Map.pdf", height = 6, width = 7, units = "cm")
...
pSGB <- ggplot(SBG) + theme_classic() + 
  ...
  theme(axis.title.x = element_blank(),
        ...
        #strip.text = element_text(colour="black", face = "bold", size = 14),
        #strip.background = element_rect(colour = NULL),
        legend.justification=c(1,0), legend.position=c(0.255, 0.0), legend.direction = "vertical",
        #panel.border = element_rect(colour = "black", fill=NA, size = 1),
        #legend.key.size = unit(1, "cm"),
        legend.text = element_text(size = 3, face = "bold"),
        legend.title=element_text(colour="black", face = "bold", size = 4),
  ...
  scalebar(NULL, x.min =boxes$x_min[3], x.max = boxes$x_max[3], y.min = boxes$y_min[3],
          y.max = boxes$y_max[3], dist = 1, dist_unit = "km",
          transform = T, model = "WGS84", st.bottom = FALSE,
          anchor = c("x" = 150.66,"y"= -35.185), border.size = 0.5, st.size = 1.5) +
  #geom_point(data = dots, aes(x = Longitude, y = Latitude*-1, col = cols), size = 2) +
  #scale_shape_manual(values = 1, label ="Larval Fish \nSample") +
  geom_point(data = SBG_reefs, aes(x = Long, y = Lat, shape = Type), size = 1) #+
#geom_text(data = boxes, aes(x = x_min, y = y_min, label = Estuary), 
#          nudge_x = c(0.75,0.55,0.8))# +

pSGB

ggsave("St Georges Basin Map.pdf", height = 6, width = 7, units = "cm")
